...
class BM25(RetrievalModel):
    def __init__(self, model_file, b=.75, k=1.2, parameters={}):
        ...
        self.index_file = f"{model_file}_b{self.b}_k{self.k}_mdf{self.min_df}_tkn{self.tokenizer}_stm{self.stemmer}_sw{self.stopwords}_lc{self.do_lowercasing}_an{self.do_ampersand_normalization}_acn{self.do_acronyms_normalization}_pr{self.do_punctuation_removal}_scn{self.do_special_chars_normalization}"
    ...
    def index(self, input_file, use_cache=True):
        ...
        with open(input_file, 'r') as f:
            ...
            se = SearchEngine(index_name=self.index_file,
                        ...
                        hyperparams={'b': self.b, 'k1': self.k}
                        ).index(data)
            se.save()
